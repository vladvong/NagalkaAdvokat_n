    ...
    const fetchSliderProjects = async () => {
      try {
        const res = await fetch(
          "https://determined-desk-f2e043cadd.strapiapp.com/api/projects?sort=createdAt:desc&populate=*",
          {
            headers: {
        ...
        const items = (json.data || []).map((item) => ({
          ...
          description:
            item.subtitle ?? item.attributes?.subtitle ?? "",
  ...
  return (
    <div className="slider-wrapper">
      ...
      <Splide
        ref={splideRef}
        options={{
          type: "loop", // ✅ infinite circular mode
          perPage: 3,
          gap: "24px",
          arrows: false,
          pagination: false,
          ...
          : projects.map((project) => {
              ...
              const imageUrl = rawImageUrl
                ? rawImageUrl.startsWith("http")
                  ? rawImageUrl
                  : `https://determined-desk-f2e043cadd.strapiapp.com${rawImageUrl}`
                : IMAGES.PROJ;
